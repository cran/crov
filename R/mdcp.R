...
#' and possibly others, Espinosa and Hennig (2018) <arXiv:1804.08715>. The parameter estimates associated with an ordinal
...
#' @seealso \code{\link[crov02:monoDirTest]{monoDirTest}}, \code{\link[stats:constrOptim]{constrOptim}}.
#' @references Espinosa J., Hennig C. "A constrained regression model for an
#' ordinal response with ordinal predictors."
#' arXiv:1804.08715, 2018.
#' @export
...
mdcp<-function(formula, data = NULL, tryAllMonoDir = FALSE, monoDir = NULL,
  ...
  if (tryAllMonoDir == TRUE) {
    ...
    TLBS2 <- 1
    StepSizeCLS2 <- 0 }
  if (tryAllMonoDir == TRUE & !is.null(monoDir)) stop("Either tryAllMonoDir or monoDir must must be specified.")
  ...
  if (is.null(monoDir)) {
    ...
    for (OP in 1 : length(q_cat_OrdPred)) {
      ...
      OP_LB <- OP_UMLE - qnorm(CLS1, 0, 1) * OP_SE
      OP_UB <- OP_UMLE + qnorm(CLS1, 0, 1) * OP_SE
    ...
    for (OP in which(MDC_IndexS1 == "Both")) {
      ...
      while (MDC_IndexS2Both[OP] == "Both" & CL > TLBS2) {
        ...
        OP_LB <- OP_UMLE - qnorm(CL, 0, 1) * OP_SE
        OP_UB <- OP_UMLE + qnorm(CL, 0, 1) * OP_SE
  ...
  for (i in 1 : num_models) {
    res.aux[[i]] <- createConstraints(matY = matY, matX = matX, paramInit = param_UMLE, q_cat_OrdPred = q_cat_OrdPred,
                                         increasing = increasing.mat[i,])
    cmle.aux[[i]] <- constrOptim(res.aux[[i]]$ConstrParamInit, f = loglikOrdPred, grad = loglikGrad,
                                 ui = res.aux[[i]]$A, ci=-res.aux[[i]]$B, control = list(reltol = 1e-05, fnscale = -1),
                                 method = "BFGS", matY = matY, matX = matX, outer.iterations = 100, mu = 1e-04, outer.eps = 1e-05)
